...
- **V9 (NOMINATED — adaptlearn sandbox, token-free):** instrument BIST (§B4) — a stuck-at
  ...
  pattern, 0.11.4).
  **ANSWERED 2026-07-13 (F24, 0.11.6): GREEN** — control 7/7, 7/7 faults detected by their
  own assertions, falsifier 8/8 sabotaged vectors miss; run 1's control arm caught a real
  fixture bug (dir-argv close redding everything, VEC-1 passing for the wrong reason)
  before anything trusted the instrument — the pre-flight demonstrating its value one
  level early. Spec shipped to bareloop `docs/plans/2026-07-13-instrument-bist-spec.md`.
