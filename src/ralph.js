// The Ralph shell — the fixed, deliberately dumb outer loop (PRD §2).
// It holds the arbiter (the close: exit code = truth) and the budget (capRuns),
// and nothing inside can negotiate with either. It is stateless across runs and
// stdlib-only by design: it must stay too dumb to be gamed. Do not make it smarter.

import { spawnSync } from 'node:child_process';

/**
 * Run a close command; its exit code is the verdict (PRD §4 mapping).
...
export function runClose(close) {
  ...
  delete env.NODE_TEST_CONTEXT; // §4: a `node --test` close inherits this and silently no-ops
...
/**
 * The loop: `while close-red and under-cap: run the middle`. Stops at first green
 * (within-run tuning past a visible close is the fit-to-pass surface). The two honest
 * terminals are green and a decision-ready escalation; cap-halt means "not under cap",
...
export async function ralph({ middle, close, capRuns, emit }) {
  ...
  for (let iteration = 1; iteration <= capRuns; iteration++) {
    ...
    gap = v.gap; // feedback for the next middle run; Ralph itself learns nothing
  }
