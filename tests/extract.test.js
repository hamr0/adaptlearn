// M6 rules extractor — token-free over a scripted stub provider (the provider
// is the shell's legitimate seam). Contract under test: one sealed shot, never
// throws, the ≤5-rules/≤200-chars bound is enforced mechanically post-call
// (rejected whole, never trimmed — the cap-touch rule: rejecting a
// half-applicable output beats silently part-applying it), malformed output is
// a red as DATA (never a silent empty inheritance), and the extractor sees
// ledger facts only — config, verdict, revision diff, prior rules — never a
// close or test text.
...
test('valid rules list → carried through with cost', async () => {
  ...
  assert.ok(r.costUsd > 0, 'extractor spend must be carried (§7b.3)');
});
